...
describe("Prompt Marketplace", () => {
    ...
    // Program ID
    const programId = new web3_js_1.PublicKey("CBrB6yQSi9pcxKuRR1uPjj6NLipfpZKYYT71c3gaFf1Y");
    // Admin keypair (loaded from ~/.config/solana/id.json)
    const admin = anchor.web3.Keypair.fromSecretKey(Buffer.from(JSON.parse(fs.readFileSync(process.env.HOME + "/.config/solana/id.json", "utf8"))));
    ...
    before(async () => {
        try {
            // Config PDA
            const configPda = new web3_js_1.PublicKey("4wzdty85maw7Q6TZE8Z496DgJeFHwcA8HzmNMbBE8ivJ");
            // Check if Config account exists
            const configAccountInfo = await withRetry(() => connection.getAccountInfo(configPda));
            if (!configAccountInfo) {
                // Build initialize instruction
                const instructionDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("global:initialize").digest().slice(0, 8));
                const instructionData = Buffer.alloc(16);
                initializeSchema.encode({
                    discriminator: instructionDiscriminator,
                    fee_bps: new bn_js_1.default(1000),
                }, instructionData);
                ...
                const tx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), initIx);
                const latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
                tx.recentBlockhash = latestBlockhash.blockhash;
                tx.feePayer = admin.publicKey;
                tx.sign(admin);
                const initTxSig = await withRetry(() => connection.sendRawTransaction(tx.serialize(), {
                    skipPreflight: false,
                ...
                await withRetry(() => connection.confirmTransaction({
                    signature: initTxSig,
            ...
            // Derive Prompt PDA
            const [promptPdaDerived, bump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("prompt"), mintPubkey.toBuffer()], programId);
            promptPda = promptPdaDerived;
            promptBump = bump;
            // Derive Metadata PDA
            const [metadataPdaDerived] = web3_js_1.PublicKey.findProgramAddressSync([
            ...
            creatorToken = (0, spl_token_1.getAssociatedTokenAddressSync)(mintPubkey, admin.publicKey, false, spl_token_1.TOKEN_PROGRAM_ID);
            ...
            const initMintIx = (0, spl_token_1.createInitializeMint2Instruction)(mintPubkey, 0, // decimals
            admin.publicKey, // mint authority
            admin.publicKey, // freeze authority
            spl_token_1.TOKEN_PROGRAM_ID);
            const createAtaIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(admin.publicKey, // payer
            creatorToken, // ATA address
            admin.publicKey, // owner
            mintPubkey, // mint
            spl_token_1.TOKEN_PROGRAM_ID);
            const initTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), web3_js_1.SystemProgram.createAccount({
                fromPubkey: admin.publicKey,
                newAccountPubkey: mintPubkey,
                space: spl_token_1.MINT_SIZE,
                lamports: await withRetry(() => connection.getMinimumBalanceForRentExemption(spl_token_1.MINT_SIZE)),
                programId: spl_token_1.TOKEN_PROGRAM_ID,
            }), initMintIx, createAtaIx);
            const latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            initTx.recentBlockhash = latestBlockhash.blockhash;
            initTx.feePayer = admin.publicKey;
            initTx.sign(admin, mint);
            const initTxSig = await withRetry(() => connection.sendRawTransaction(initTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!initTxSig) {
            ...
            // Create prompt
            const instructionDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("global:create_prompt").digest().slice(0, 8));
            const instructionData = Buffer.alloc(8 + 4 + metadataUri.length + 8);
            const encodedLength = createPromptSchema.encode({
                discriminator: instructionDiscriminator,
                metadata_uri: metadataUri,
                royalty_bps: new bn_js_1.default(royaltyBps),
            }, instructionData);
            ...
            const createIx = new web3_js_1.TransactionInstruction({
                keys: [
                    ...
                    { pubkey: new web3_js_1.PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"), isSigner: false, isWritable: false }, // metadata_program
                    { pubkey: anchor.web3.SYSVAR_RENT_PUBKEY, isSigner: false, isWritable: false }, // rent
                ],
                programId,
                data: trimmedInstructionData,
            });
            const createTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 800000 }), createIx);
            ...
            createTx.sign(admin);
            const createTxSig = await withRetry(() => connection.sendRawTransaction(createTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!createTxSig) {
    ...
    it("Initializes the marketplace", async () => {
        try {
            ...
            (0, chai_1.assert)(accountInfo, "Config account should exist");
            ...
            (0, chai_1.assert)(accountDiscriminator.equals(expectedDiscriminator), `Invalid discriminator: expected ${expectedDiscriminator.toString("hex")}, got ${accountDiscriminator.toString("hex")}`);
            chai_1.assert.equal(decoded.admin.toBase58(), admin.publicKey.toBase58(), "Admin should match");
            chai_1.assert.equal(Number(decoded.fee_bps), 1000, "Fee should be 1000 bps");
            chai_1.assert.equal(decoded.bump, 255, "Bump should match");
        }
        catch (err) {
    ...
    it("Creates a prompt", async () => {
        var _a, _b, _c, _d;
        try {
            const expectedDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("account:Prompt").digest().slice(0, 8));
            const accountInfo = await withRetry(() => connection.getAccountInfo(promptPda));
            (0, chai_1.assert)(accountInfo, "Prompt account should exist");
            ...
            const accountDiscriminator = accountInfo.data.slice(0, 8);
            (0, chai_1.assert)(accountDiscriminator.equals(expectedDiscriminator), `Invalid discriminator: expected ${expectedDiscriminator.toString("hex")}, got ${accountDiscriminator.toString("hex")}`);
            chai_1.assert.equal(decoded.mint.toBase58(), mintPubkey.toBase58(), "Mint should match");
            chai_1.assert.equal(decoded.creator.toBase58(), admin.publicKey.toBase58(), "Creator should match");
            chai_1.assert.equal(decoded.metadata_uri, "https://example.com/metadata.json", "Metadata URI should match");
            chai_1.assert.equal(Number(decoded.royalty_bps), 500, "Royalty BPS should match");
            chai_1.assert.equal(decoded.bump, promptBump, "Bump should match");
            console.log(`Prompt Bump: expected ${promptBump}, actual ${decoded.bump}`);
            // Verify mint state
            const mintInfo = await withRetry(() => (0, spl_token_1.getMint)(connection, mintPubkey));
            console.log("Mint Info:", {
                address: mintPubkey.toBase58(),
                mintAuthority: ((_a = mintInfo.mintAuthority) === null || _a === void 0 ? void 0 : _a.toBase58()) || "null",
                freezeAuthority: ((_b = mintInfo.freezeAuthority) === null || _b === void 0 ? void 0 : _b.toBase58()) || "null",
                decimals: mintInfo.decimals,
                supply: mintInfo.supply.toString()
            });
            chai_1.assert.equal((_c = mintInfo.mintAuthority) === null || _c === void 0 ? void 0 : _c.toBase58(), admin.publicKey.toBase58(), "Mint authority should be admin");
            chai_1.assert.equal((_d = mintInfo.freezeAuthority) === null || _d === void 0 ? void 0 : _d.toBase58(), admin.publicKey.toBase58(), "Freeze authority should be admin");
            chai_1.assert.equal(mintInfo.decimals, 0, "Mint decimals should be 0");
            chai_1.assert.equal(mintInfo.supply.toString(), "1", "Mint supply should be 1");
            // Verify creator token account
            const creatorTokenInfo = await withRetry(() => (0, spl_token_1.getAccount)(connection, creatorToken));
            ...
            chai_1.assert.equal(creatorTokenInfo.mint.toBase58(), mintPubkey.toBase58(), "Creator token mint should match");
            chai_1.assert.equal(creatorTokenInfo.owner.toBase58(), admin.publicKey.toBase58(), "Creator token owner should be admin");
            chai_1.assert.equal(creatorTokenInfo.amount.toString(), "1", "Creator token amount should be 1");
        }
        catch (err) {
            console.error("Test error:", err);
            throw err;
    ...
    it("Lists and delists a prompt", async () => {
        try {
            ...
            const price = new bn_js_1.default(1000000000); // 1 SOL
            // Derive Listing PDA
            const [listingPda, listingBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("listing"), mintPubkey.toBuffer()], programId);
            console.log("Listing PDA:", listingPda.toBase58());
            // Derive Escrow Authority PDA
            const [escrowAuthority, escrowBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("escrow"), mintPubkey.toBuffer()], programId);
            console.log("Escrow Authority PDA:", escrowAuthority.toBase58());
            // Compute Escrow Token Account (ATA) address
            const escrowToken = (0, spl_token_1.getAssociatedTokenAddressSync)(mintPubkey, escrowAuthority, true, // Allow owner off-curve (PDA)
            spl_token_1.TOKEN_PROGRAM_ID);
            console.log("Escrow Token Address:", escrowToken.toBase58());
            // Expected discriminator for Listing account
            const expectedDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("account:Listing").digest().slice(0, 8));
            ...
            if (!escrowAccountInfo) {
                ...
                const createEscrowAtaIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(seller.publicKey, // payer
                escrowToken, // ATA address
                escrowAuthority, // owner (PDA)
                mintPubkey, // mint
                spl_token_1.TOKEN_PROGRAM_ID);
                const initEscrowTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 600000 }), createEscrowAtaIx);
                ...
                initEscrowTxSig = await withRetry(() => connection.sendRawTransaction(initEscrowTx.serialize(), {
                    skipPreflight: false,
                    preflightCommitment: "confirmed",
                }));
                if (!initEscrowTxSig) {
            ...
            const escrowTokenInfoInit = await withRetry(() => (0, spl_token_1.getAccount)(connection, escrowToken));
            ...
            chai_1.assert.equal(escrowTokenInfoInit.mint.toBase58(), mintPubkey.toBase58(), "Escrow token mint should match");
            chai_1.assert.equal(escrowTokenInfoInit.owner.toBase58(), escrowAuthority.toBase58(), "Escrow token owner should be escrow authority");
            chai_1.assert.equal(escrowTokenInfoInit.amount.toString(), "0", "Escrow token amount should be 0");
            // Build list_prompt instruction
            const listInstructionDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("global:list_prompt").digest().slice(0, 8));
            const listInstructionData = Buffer.alloc(16);
            listPromptSchema.encode({
                discriminator: listInstructionDiscriminator,
                price: price
            }, listInstructionData);
            const listIx = new web3_js_1.TransactionInstruction({
                keys: [
            ...
            const listTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 400000 }), listIx);
            let latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            listTx.recentBlockhash = latestBlockhash.blockhash;
            listTx.feePayer = seller.publicKey;
            listTx.sign(seller);
            const listTxSig = await withRetry(() => connection.sendRawTransaction(listTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!listTxSig) {
            ...
            (0, chai_1.assert)(listingAccountInfo, "Listing account should exist");
            ...
            (0, chai_1.assert)(listingDiscriminator.equals(expectedDiscriminator), `Invalid discriminator: expected ${expectedDiscriminator.toString("hex")}, got ${listingDiscriminator.toString("hex")}`);
            chai_1.assert.equal(decodedListing.mint.toBase58(), mintPubkey.toBase58(), "Mint should match");
            chai_1.assert.equal(decodedListing.seller.toBase58(), seller.publicKey.toBase58(), "Seller should match");
            chai_1.assert.equal(Number(decodedListing.price), price.toNumber(), "Price should match");
            chai_1.assert.equal(decodedListing.is_active, true, "Listing should be active");
            chai_1.assert.equal(decodedListing.bump, listingBump, "Bump should match");
            // Verify Escrow Token account
            const escrowTokenInfo = await withRetry(() => (0, spl_token_1.getAccount)(connection, escrowToken));
            ...
            chai_1.assert.equal(escrowTokenInfo.mint.toBase58(), mintPubkey.toBase58(), "Escrow token mint should match");
            chai_1.assert.equal(escrowTokenInfo.owner.toBase58(), escrowAuthority.toBase58(), "Escrow token owner should be escrow authority");
            chai_1.assert.equal(escrowTokenInfo.amount.toString(), "1", "Escrow token amount should be 1");
            // Verify Seller Token account
            const sellerTokenInfo = await withRetry(() => (0, spl_token_1.getAccount)(connection, creatorToken));
            ...
            chai_1.assert.equal(sellerTokenInfo.mint.toBase58(), mintPubkey.toBase58(), "Seller token mint should match");
            chai_1.assert.equal(sellerTokenInfo.owner.toBase58(), seller.publicKey.toBase58(), "Seller token owner should be seller");
            chai_1.assert.equal(sellerTokenInfo.amount.toString(), "0", "Seller token amount should be 0");
            // Build delist_prompt instruction
            const delistInstructionDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("global:delist_prompt").digest().slice(0, 8));
            ...
            const delistIx = new web3_js_1.TransactionInstruction({
                keys: [
                    { pubkey: listingPda, isSigner: false, isWritable: true }, // listing
                    { pubkey: promptPda, isSigner: false, isWritable: false }, // prompt
                    { pubkey: mintPubkey, isSigner: false, isWritable: false }, // mint
            ...
            const delistTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 400000 }), delistIx);
            ...
            const delistTxSig = await withRetry(() => connection.sendRawTransaction(delistTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!delistTxSig) {
            ...
            (0, chai_1.assert)(listingAccountInfoPost, "Listing account should exist");
            ...
            chai_1.assert.equal(decodedListingPost.is_active, false, "Listing should be inactive");
            // Verify Escrow Token account (after delisting)
            const escrowTokenInfoPost = await withRetry(() => (0, spl_token_1.getAccount)(connection, escrowToken));
            ...
            chai_1.assert.equal(escrowTokenInfoPost.amount.toString(), "0", "Escrow token amount should be 0");
            // Verify Seller Token account (after delisting)
            const sellerTokenInfoPost = await withRetry(() => (0, spl_token_1.getAccount)(connection, creatorToken));
            ...
            chai_1.assert.equal(sellerTokenInfoPost.amount.toString(), "1", "Seller token amount should be 1");
        }
        catch (err) {
            console.error("Test error:", err);
            throw err;
    ...
    it("Buys and re-lists a prompt", async () => {
        ...
        try {
            ...
            const seller = web3_js_1.Keypair.fromSecretKey(Buffer.from(JSON.parse(fs.readFileSync(process.env.HOME + "/Documents/projects/Rust/solana/partnership/prompt-hub-sol/prompt_marketplace/seller-keypair.json", "utf8"))));
            ...
            console.log("Seller SOL Balance (initial):", sellerBalance / web3_js_1.LAMPORTS_PER_SOL, "SOL");
            if (sellerBalance < 0.1 * web3_js_1.LAMPORTS_PER_SOL) {
                throw new Error(`Seller wallet ${seller.publicKey.toBase58()} has insufficient SOL (${sellerBalance / web3_js_1.LAMPORTS_PER_SOL} SOL). ` +
            ...
            const sellerToken = (0, spl_token_1.getAssociatedTokenAddressSync)(mintPubkey, seller.publicKey, false, spl_token_1.TOKEN_PROGRAM_ID);
            // Create seller's token account
            const createSellerAtaIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(admin.publicKey, // payer
            sellerToken, seller.publicKey, mintPubkey, spl_token_1.TOKEN_PROGRAM_ID);
            // Transfer NFT from admin to seller
            const transferIx = (0, spl_token_1.createTransferInstruction)(creatorToken, sellerToken, admin.publicKey, 1, [], spl_token_1.TOKEN_PROGRAM_ID);
            const transferTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), createSellerAtaIx, transferIx);
            let latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            transferTx.recentBlockhash = latestBlockhash.blockhash;
            transferTx.feePayer = admin.publicKey;
            transferTx.sign(admin);
            const transferTxSig = await withRetry(() => connection.sendRawTransaction(transferTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!transferTxSig) {
            ...
            const buyer = web3_js_1.Keypair.fromSecretKey(Buffer.from(JSON.parse(fs.readFileSync(process.env.HOME + "/Documents/projects/Rust/solana/partnership/prompt-hub-sol/prompt_marketplace/buyer-keypair.json", "utf8"))));
            ...
            console.log("Buyer SOL Balance (initial):", buyerBalance / web3_js_1.LAMPORTS_PER_SOL, "SOL");
            if (buyerBalance < 1.1 * web3_js_1.LAMPORTS_PER_SOL) {
                throw new Error(`Buyer wallet ${buyer.publicKey.toBase58()} has insufficient SOL (${buyerBalance / web3_js_1.LAMPORTS_PER_SOL} SOL). ` +
            ...
            const buyerToken = (0, spl_token_1.getAssociatedTokenAddressSync)(mintPubkey, buyer.publicKey, false, spl_token_1.TOKEN_PROGRAM_ID);
            ...
            let initBuyerAtaTxSig;
            if (!buyerTokenInfoPre) {
                const createBuyerAtaIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(buyer.publicKey, // payer
                buyerToken, // ATA address
                buyer.publicKey, // owner
                mintPubkey, // mint
                spl_token_1.TOKEN_PROGRAM_ID);
                const initBuyerAtaTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), createBuyerAtaIx);
                latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
                initBuyerAtaTx.recentBlockhash = latestBlockhash.blockhash;
                initBuyerAtaTx.feePayer = buyer.publicKey;
                initBuyerAtaTx.sign(buyer);
                initBuyerAtaTxSig = await withRetry(() => connection.sendRawTransaction(initBuyerAtaTx.serialize(), {
                    skipPreflight: false,
                    preflightCommitment: "confirmed",
                }));
            ...
            const buyerTokenState = await withRetry(() => (0, spl_token_1.getAccount)(connection, buyerToken));
            console.log("Buyer Token Info (pre-buy):", {
                ...
                delegate: ((_a = buyerTokenState.delegate) === null || _a === void 0 ? void 0 : _a.toBase58()) || "null",
                delegatedAmount: buyerTokenState.delegatedAmount.toString()
            });
            chai_1.assert.equal(buyerTokenState.mint.toBase58(), mintPubkey.toBase58(), "Buyer token mint should match");
            chai_1.assert.equal(buyerTokenState.owner.toBase58(), buyer.publicKey.toBase58(), "Buyer token owner should be buyer");
            chai_1.assert.equal(buyerTokenState.amount.toString(), "0", "Buyer token amount should be 0 before purchase");
            ...
            console.log("Initial SOL Balances:", {
                seller: {
                    ...
                    balance: sellerBalanceBefore / web3_js_1.LAMPORTS_PER_SOL,
                    lamports: sellerBalanceBefore
                },
                admin: {
                    publicKey: admin.publicKey.toBase58(),
                    balance: adminBalanceBefore / web3_js_1.LAMPORTS_PER_SOL,
                    lamports: adminBalanceBefore
                },
                creator: {
                    publicKey: admin.publicKey.toBase58(),
                    balance: creatorBalanceBefore / web3_js_1.LAMPORTS_PER_SOL,
                    lamports: creatorBalanceBefore
                },
                buyer: {
                    publicKey: buyer.publicKey.toBase58(),
                    balance: buyerBalanceBefore / web3_js_1.LAMPORTS_PER_SOL,
                    lamports: buyerBalanceBefore
                }
            });
            // Build buy_prompt instruction
            const buyInstructionDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("global:buy_prompt").digest().slice(0, 8));
            ...
            const buyIxAccounts = [
                ...
                { pubkey: new web3_js_1.PublicKey("4wzdty85maw7Q6TZE8Z496DgJeFHwcA8HzmNMbBE8ivJ"), isSigner: false, isWritable: false, name: "config" },
                ...
                { pubkey: web3_js_1.SystemProgram.programId, isSigner: false, isWritable: false, name: "system_program" },
                { pubkey: spl_token_1.TOKEN_PROGRAM_ID, isSigner: false, isWritable: false, name: "token_program" },
                { pubkey: new web3_js_1.PublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL"), isSigner: false, isWritable: false, name: "associated_token_program" }
            ];
            ...
            const buyTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 1000000 }), // Reverted to 1,000,000
            buyIx);
            latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            ...
            const buyTxSig = await withRetry(() => connection.sendRawTransaction(buyTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!buyTxSig) {
            ...
            (0, chai_1.assert)(listingAccountInfoPostBuy, "Listing account should exist");
            ...
            chai_1.assert.equal(decodedListingPostBuy.is_active, false, "Listing should be inactive");
            // Verify Escrow Token account (after purchase)
            const escrowTokenInfoPostBuy = await withRetry(() => (0, spl_token_1.getAccount)(connection, escrowToken));
            ...
            chai_1.assert.equal(escrowTokenInfoPostBuy.amount.toString(), "0", "Escrow token amount should be 0");
            // Verify Buyer Token account (after purchase)
            const buyerTokenInfoPostBuy = await withRetry(() => (0, spl_token_1.getAccount)(connection, buyerToken));
            ...
            chai_1.assert.equal(buyerTokenInfoPostBuy.mint.toBase58(), mintPubkey.toBase58(), "Buyer token mint should match");
            chai_1.assert.equal(buyerTokenInfoPostBuy.owner.toBase58(), buyer.publicKey.toBase58(), "Buyer token owner should be buyer");
            chai_1.assert.equal(buyerTokenInfoPostBuy.amount.toString(), "1", "Buyer token amount should be 1");
            ...
            console.log("SOL Balances After Purchase:", {
                seller: {
                    publicKey: seller.publicKey.toBase58(),
                    balance: sellerBalanceAfter / web3_js_1.LAMPORTS_PER_SOL,
                ...
                admin: {
                    publicKey: admin.publicKey.toBase58(),
                    balance: adminBalanceAfter / web3_js_1.LAMPORTS_PER_SOL,
                ...
                creator: {
                    publicKey: admin.publicKey.toBase58(),
                    balance: creatorBalanceAfter / web3_js_1.LAMPORTS_PER_SOL,
                ...
                buyer: {
                    publicKey: buyer.publicKey.toBase58(),
                    balance: buyerBalanceAfter / web3_js_1.LAMPORTS_PER_SOL,
            ...
            chai_1.assert.approximately(sellerBalanceAfter, sellerBalanceBefore + sellerAmount, 100000, // Allow 0.0001 SOL variance for fees
            `Seller balance should increase by seller amount (${sellerAmount} lamports)`);
            chai_1.assert.approximately(adminBalanceAfter, adminBalanceBefore + platformFee + royalty, // Admin receives both (creator is admin)
            100000, `Admin balance should increase by platform fee (${platformFee}) + royalty (${royalty})`);
            chai_1.assert.approximately(buyerBalanceAfter, buyerBalanceBefore - price.toNumber(), 1000000, // Increased tolerance to 0.001 SOL to account for transaction fees
            `Buyer balance should decrease by price (${price.toNumber()} lamports)`);
            // Re-list prompt (buyer becomes seller)
            const newSeller = buyer;
            const newPrice = new bn_js_1.default(1500000000); // 1.5 SOL
            ...
            const relistIx = new web3_js_1.TransactionInstruction({
                keys: [
                    { pubkey: listingPda, isSigner: false, isWritable: true }, // listing
                    { pubkey: promptPda, isSigner: false, isWritable: false }, // prompt
                    { pubkey: mintPubkey, isSigner: false, isWritable: false }, // mint
            ...
            const relistTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 400000 }), relistIx);
            latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            relistTx.recentBlockhash = latestBlockhash.blockhash;
            relistTx.feePayer = newSeller.publicKey;
            relistTx.sign(newSeller);
            const relistTxSig = await withRetry(() => connection.sendRawTransaction(relistTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            if (!relistTxSig) {
            ...
            (0, chai_1.assert)(listingAccountInfoPostRelist, "Listing account should exist");
            ...
            chai_1.assert.equal(decodedListingPostRelist.mint.toBase58(), mintPubkey.toBase58(), "Mint should match");
            chai_1.assert.equal(decodedListingPostRelist.seller.toBase58(), newSeller.publicKey.toBase58(), "Seller should be buyer");
            chai_1.assert.equal(Number(decodedListingPostRelist.price), newPrice.toNumber(), "Price should match new price");
            chai_1.assert.equal(decodedListingPostRelist.is_active, true, "Listing should be active");
            chai_1.assert.equal(decodedListingPostRelist.bump, listingBump, "Bump should match");
            // Verify Escrow Token account (after re-listing)
            const escrowTokenInfoPostRelist = await withRetry(() => (0, spl_token_1.getAccount)(connection, escrowToken));
            ...
            chai_1.assert.equal(escrowTokenInfoPostRelist.amount.toString(), "1", "Escrow token amount should be 1");
            // Verify Buyer Token account (after re-listing)
            const buyerTokenInfoPostRelist = await withRetry(() => (0, spl_token_1.getAccount)(connection, buyerToken));
            ...
            chai_1.assert.equal(buyerTokenInfoPostRelist.amount.toString(), "0", "Buyer token amount should be 0");
        }
        catch (err) {
            console.error("Test error:", err);
            throw err;
    ...
    it("Fails to buy prompt with insufficient funds", async () => {
        try {
            ...
            console.log("Escrow Authority PDA:", escrowAuthority.toBase58());
            // Compute Escrow Token Account (ATA) address
            const escrowToken = (0, spl_token_1.getAssociatedTokenAddressSync)(mintPubkey, escrowAuthority, true, spl_token_1.TOKEN_PROGRAM_ID);
            console.log("Escrow Token Address:", escrowToken.toBase58());
            // Compute Buyer Token Account (ATA) address
            const buyerToken = (0, spl_token_1.getAssociatedTokenAddressSync)(mintPubkey, buyer.publicKey, false, spl_token_1.TOKEN_PROGRAM_ID);
            console.log("Buyer Token Address:", buyerToken.toBase58());
            ...
            let initBuyerAtaTxSig;
            if (!buyerTokenInfo) {
                const createBuyerAtaIx = (0, spl_token_1.createAssociatedTokenAccountInstruction)(buyer.publicKey, buyerToken, buyer.publicKey, mintPubkey, spl_token_1.TOKEN_PROGRAM_ID);
                const initBuyerAtaTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), createBuyerAtaIx);
                let latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
                initBuyerAtaTx.recentBlockhash = latestBlockhash.blockhash;
                initBuyerAtaTx.feePayer = buyer.publicKey;
            ...
            chai_1.assert.equal(buyerTokenState.amount.toString(), "1", "Buyer token amount should be 1 after delisting");
            // Transfer the prompt NFT from buyer back to seller
            const transferBackIx = (0, spl_token_1.createTransferInstruction)(buyerToken, sellerToken, buyer.publicKey, 1, [], spl_token_1.TOKEN_PROGRAM_ID);
            const transferBackTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), transferBackIx);
            latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            transferBackTx.recentBlockhash = latestBlockhash.blockhash;
            transferBackTx.feePayer = buyer.publicKey;
            transferBackTx.sign(buyer);
            const transferBackTxSig = await withRetry(() => connection.sendRawTransaction(transferBackTx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "confirmed",
            }));
            ...
            const buyerBalance = await withRetry(() => connection.getBalance(buyer.publicKey));
            console.log("Buyer SOL Balance (before reduction):", buyerBalance / web3_js_1.LAMPORTS_PER_SOL, "SOL");
            if (buyerBalance > 0.05 * web3_js_1.LAMPORTS_PER_SOL) {
                const tempKeypair = web3_js_1.Keypair.generate();
                const transferAmount = buyerBalance - Math.floor(0.04 * web3_js_1.LAMPORTS_PER_SOL); // Leave ~0.04 SOL
                console.log("Transferring", transferAmount / web3_js_1.LAMPORTS_PER_SOL, "SOL to temp account:", tempKeypair.publicKey.toBase58());
                const transferIx = web3_js_1.SystemProgram.transfer({
                    fromPubkey: buyer.publicKey,
                    toPubkey: tempKeypair.publicKey,
                    lamports: transferAmount,
                });
                const reduceBalanceTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 200000 }), transferIx);
                latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
                ...
                const reduceTxSig = await withRetry(() => connection.sendRawTransaction(reduceBalanceTx.serialize(), {
                    skipPreflight: false,
                    preflightCommitment: "confirmed",
                }));
                if (!reduceTxSig) {
            ...
            console.log("Buyer SOL Balance (after reduction):", buyerBalanceAfterReduction / web3_js_1.LAMPORTS_PER_SOL, "SOL");
            (0, chai_1.assert)(buyerBalanceAfterReduction < price.toNumber(), "Buyer balance should be less than price");
            // Build buy_prompt instruction
            const buyInstructionDiscriminator = Buffer.from((0, crypto_1.createHash)("sha256").update("global:buy_prompt").digest().slice(0, 8));
            const buyInstructionData = Buffer.alloc(8);
            buyPromptSchema.encode({
            ...
            const buyIx = new web3_js_1.TransactionInstruction({
                keys: [
                    { pubkey: listingPda, isSigner: false, isWritable: true },
                    { pubkey: promptPda, isSigner: false, isWritable: false },
                    { pubkey: new web3_js_1.PublicKey("4wzdty85maw7Q6TZE8Z496DgJeFHwcA8HzmNMbBE8ivJ"), isSigner: false, isWritable: false },
                    { pubkey: mintPubkey, isSigner: false, isWritable: false },
            ...
            const buyTx = new web3_js_1.Transaction().add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 200000 }), web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 1000000 }), buyIx);
            latestBlockhash = await withRetry(() => connection.getLatestBlockhash("confirmed"));
            buyTx.recentBlockhash = latestBlockhash.blockhash;
            buyTx.feePayer = buyer.publicKey;
            buyTx.sign(buyer);
            ...
            let error;
            try {
                buyTxSig = await withRetry(() => connection.sendRawTransaction(buyTx.serialize(), {
                    skipPreflight: false,
                    preflightCommitment: "confirmed",
                }));
                if (!buyTxSig) {
            ...
            (0, chai_1.assert)(error, "Buy transaction should have failed due to insufficient funds");
            (0, chai_1.assert)(!buyTxSig || error, "Transaction should not have succeeded");
            // Verify buyer token account state (should not have received the NFT)
            const buyerTokenStatePost = await withRetry(() => (0, spl_token_1.getAccount)(connection, buyerToken));
            ...
            chai_1.assert.equal(buyerTokenStatePost.amount.toString(), "0", "Buyer token amount should remain 0");
        }
        catch (err) {
            console.error("Test error:", err);
            throw err;
